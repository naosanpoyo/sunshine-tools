...
const Page = async () => {
    ...
    let data = null;
    try {
        const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&eventType=live&type=video&maxResults=10&key=${process.env.YOUTUBE_KEY}&q=${process.env.YOUTUBE_QUERY}`, { next: { revalidate: 600 } });
        data = await res.json();
        console.log(data);
    } catch (error) {
        console.log(error);
